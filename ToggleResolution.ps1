# Set paths to a universal location
$baseDir = "C:\ProgramData\ResolutionToggle"
$qresPath = "$baseDir\QRes.exe"
$logDir = "$baseDir\logs"
...
if (-Not (Test-Path $qresPath)) {
    Write-Log "ERROR: QRes.exe not found! Make sure it's in $baseDir"
    exit
}
